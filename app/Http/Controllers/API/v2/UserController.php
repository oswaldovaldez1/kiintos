    ...
    private function gethistorial($trancciones, $usuario) {
        ...
        foreach ($trancciones as $histo) {
            ...
            if ($histo['id_c2'] === 0) {
                ...
                if(Detalleusuario::where('idcuenta', '=', $histo['id_c2'])->count()>0){
            ...
            if ($histo['entrada'] == 0) {
                ...
                if ($histo['tipot'] === 'I') {
                    array_push($respuesta, array('titulo' => 'Ingreso', 'color' => \config('confkiintos')['colors']['AZUL'], 'mensaje' => 'Ingreso por $ ' . number_format($this->desencriptar($histo['cantidad']), 2), 'descripcion' => $histo['descripcion'], 'perfil' => $perfil, 'fecha' => $histo['fecha']));
                }
            }
        }
        return $respuesta;
