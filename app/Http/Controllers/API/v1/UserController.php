...
use App\Models\v1\Detalleusuario;
use App\Models\v1\Saldo;
use App\User;
use App\Models\v1\Catredes;
...
class UserController extends Controller {
    ...
    public function updateprofile() {
        ...
        if (request()->header('tcuenta') === 'personal') {
            ...
            \App\Models\v1\Empresa::where('id_empresa', '=', $usuario->get()->toArray()[0]['id'])->update(['direccion' => request()->header('direccion'), 'servicios' => request()->header('servicios'), 'tellocal' => request()->header('tellocal'), 'url' => request()->header('url'), 'ubicacion' => '', 'horario_atencion' => request()->header('horarios')]);

            $redes = Catredes::all();
            foreach ($redes as $reds) {
                \App\Models\v1\Redes::where('id_empresa', '=', $usuario->get()->toArray()[0]['id'])->where('id_redes', '=', $reds->id)->update(['url' => $redesoc[$reds->nombre]]);
            }
            $this->log(json_encode(array('Movimiento' => 'Actulaización Cuenta',
                "Detalle" => array('Id' => $usuario->get()->toArray()[0]['id'], 'TCuenta' => $usuario->get()->toArray()[0]['esempresa']),
                'Fecha' => date("Y-m-d H:i:s")), JSON_UNESCAPED_SLASHES));
    ...
    public function record() {
        ...
        $transacciones = \App\Models\v1\Transaccione::where('id_cliente', '=', $usuario->get()->toArray()[0]['id'])
                        ->where('status', '<>', 'C')
    ...
    public function newuser() {
        ...
        $user = Usuario::find(request()->header('correo'));
        if ($user <> null) {
        ...
        if ($user <> null) {
            ...
            $detalleusuario->f_nacimiento = date('Y-m-d H:i:s', mktime(0, 0, 0, 12, 31, 1800));
            ...
            if ($user->esempresa === 1) {
                $empresa = new \App\Models\v1\Empresa();
                ...
                $empresa->ubicacion = '';
                $empresa->save();
                $emp = \App\Models\v1\Empresa::find($user->id);
                if ($empresa === null) {
                    Detalleusuario::where('idcuenta', '=', $user->id)->delete();
    ...
    public function notice() {
        ...
        $notificaciones = \App\Models\v1\Notificacion::where('status', '=', 'P')
                ->where('idnot', '=', $usuario->get()->toArray()[0]['id'])
                ->limit(1);

        if ($notificaciones->count() > 0) {
            $notificaciones = $notificaciones->get()->toArray()[0];
            \App\Models\v1\Notificacion::where('id', '=', $notificaciones['id'])->update(['status' => 'V']);
    ...
    public function answer() {
        ...
        $not = \App\Models\v1\Notificacion::where('id', '=', request()->header('id'));
        if ($not->count() === 0) {
            return response()->json(['message' => 'Sus credenciales no se pudierón verificar', 'status' => $this->notfoudStatus], $this->notfoudStatus, [], JSON_UNESCAPED_SLASHES);
        }
        ...
        switch (request()->header('respuesta')) {
            case "s": {
                    ...
                    $tran = \App\Models\v1\Transaccione::where('fecha', '=', $not['fecha'])->where('id_cliente', '=', $not['idnot']);
                    ...
                    \App\Models\v1\Transaccione::where('fecha', '=', $not['fecha'])->where('id_cliente', '=', $not['idnot'])->update(['status' => 'R']);
                    \App\Models\v1\Transaccione::where('fecha', '=', $not['fecha'])->where('id_cliente', '=', $tran['id_c2'])->update(['status' => 'R']);
                    ...
                    $not = new \App\Models\v1\Notificacion();
            ...
            case "n": {
                    ...
                    \App\Models\v1\Transaccione::where('fecha', '=', $not['fecha'])->where('id_cliente', '=', $not['idnot'])->update(['status' => 'C']);
                    \App\Models\v1\Transaccione::where('fecha', '=', $not['fecha'])->where('id_cliente', '=', $tran['id_c2'])->update(['status' => 'C']);
    ...
    private function getperfil($id) {
        ...
        if ($tcuenta['esempresa'] === 0) {
            $dperfil = Usuario::select('usuario.correo', 'usuario.esempresa as tipo_cuenta', 'detalleusuario.nombre', 'detalleusuario.apellidos', 'detalleusuario.telefono', 'detalleusuario.perfil', 'detalleusuario.f_nacimiento as fnacimiento', 'detalleusuario.frase')
                            ...
                            ->where('usuario.id', '=', $id)
                            ->get()->toArray()[0];
            $dperfil["fnacimiento"] = ($dperfil["fnacimiento"] === '1800-12-31') ? "" : date("d-m-Y", strtotime($dperfil["fnacimiento"]));
        ...
        foreach ($trancciones as $histo) {        
            ...
            if ($histo['entrada'] == 0) {
                ...
                if ($histo['tipot'] === 'R' && $histo['status'] === 'R') {
                    ...
                    if (\App\Models\v1\Notificacion::where('idnot', '=', $usuario->get()->toArray()[0]['id'])->where('fecha', '=', $histo['fecha'])->where('cantidad', '=', $this->desencriptar($histo['cantidad']))->count() > 0) {
                        array_push($respuesta, array('titulo' => 'Egreso Pendiente', 'color' => \config('confkiintos')['colors']['AMARILLO2'], 'mensaje' => 'Egreso pendiente a ' . $histo['nombre'] . ' por $ ' . number_format($this->desencriptar($histo['cantidad']), 2),'descripcion'=>$histo['descripcion'],'perfil'=>$perfil, 'fecha' => $histo['fecha'],
                            'id' => \App\Models\v1\Notificacion::where('idnot', '=', $usuario->get()->toArray()[0]['id'])->where('fecha', '=', $histo['fecha'])->where('cantidad', '=', $this->desencriptar($histo['cantidad']))->get()->toArray()[0]['id']));
                    }
                }
