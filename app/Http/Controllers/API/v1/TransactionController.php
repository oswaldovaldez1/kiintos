...
/*modelos*/
use App\Models\v1\Transaccione;
use App\Models\v1\Usuario;
use App\Models\v1\Notificacion;
...
/*-----------------------------------------------------------------------------------------------------*/
    ...
    public function genereratecode(){
        ...
        $codigos=new \App\Models\v1\Codes();
        $codigos->codes=$cadena;
        $codigos->status=false;
        $codigos->fecha=date("Y-m-d H:i:s");
        $codigos->save();
        $codigos= \App\Models\v1\Codes::where('codes','=',$cadena)->get()->toArray()[0];
    ...
    public function genereratecodef(){
        ...
        $codigos=new \App\Models\v1\codef();
        $codigos->codigo=$cadena;
        $codigos->status=TRUE;
        $codigos->save();

        $codigos= \App\Models\v1\codef::where('codigo','=',$cadena)->get()->toArray()[0];
    ...
    public function readcode(){
        ...
        if($usuario->count()===0){
            ...
            if($cadena['tipo']==='n'){
                        $codigos= \App\Models\v1\Codes::where('id','=', $cadena['id']);
                if($codigos->count()===0){
                    return response()->json(['message'=>'Sus credenciales no se pudierón verificar','status'=>$this->notfoudStatus],$this->notfoudStatus,[],JSON_UNESCAPED_SLASHES);
                }
                ...
                if((time()-$cadenaqr['elaborado'])>300){
                    \App\Models\v1\Codes::where('id','=', $cadena['id'])->update(['status'=>TRUE]);
                ...
                $codigos= \App\Models\v1\codef::where('id','=',$cadena['id']);
                if($codigos->count()<=0){
                ...
                $res= \App\Models\v1\Detalleusuario::where('institucion','like','%.'.$param['id'].'.%')->where('idcuenta','=',$usuario->get()->toArray()[0]['id']);
    ...
    public function charge(){
        ...
        $deu= \App\Models\v1\Detalleusuario::where('idcuenta','=',$usuario->get()->toArray()[0]['id'])->get()->toArray()[0];
        ...
        try {
            ...
            if($payment->getState()=='approved'){
                ...
                $monto = $respuesta['transactions'][0]['amount']['total'];
                   ...
                   $transacciones->cantidad= $this->encriptar($usuario['crypt'], $this->remplazar($monto));
    ...
    public function payout(){
        ...
        $detalle= \App\Models\v1\Detalleusuario::where('idcuenta','=',$usuario['id'])->get()->toArray()[0];
...
private function encriptar($key,$pass){
    ...
    private function pagos($emisor,$receptor,$cantidad,$tipo1,$tipo2,$status){
        ...
        $detu1= \App\Models\v1\Detalleusuario::where('idcuenta','=',$emisor['id'])->get()->toArray()[0];
        $detu2= \App\Models\v1\Detalleusuario::where('idcuenta','=',$receptor['id'])->get()->toArray()[0];
    ...
    /*public function saldosx(){
        $info= \App\Models\v1\Detalleusuario::select('detalleusuario.idcuenta','detalleusuario.nombre','saldos.saldo')
